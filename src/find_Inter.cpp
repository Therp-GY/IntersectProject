...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		if (type == 'L') {
			...
			Point p2(x2, y2);
			Line l(p1, p2);
			find_intersection_l_l(l, line_list, point_map);
			find_intersection_l_c(l, circle_list, point_map);
			...
			cin >> r;
			Circle circle(p1, r);
			find_intersection_c_l(circle, line_list, point_map);
			find_intersection_c_c(circle, circle_list, point_map);
